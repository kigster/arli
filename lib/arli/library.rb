...
module Arli
  module Library
    ADDITIONAL_KEYS = %i[depends headers_only folder].freeze
      ...
      ::Arduino::Library::Model.from(lib).tap do |model|
        if model.nil?
          ...
          raise Arli::Errors::LibraryNotFound, 'Error: '.bold.red +
                                               ...
                                               %(  HINT: run #{"arli search 'name: /#{lib_output}/'".green}\n) +
                                               %(        to find the exact name of the library you are trying\n) +
                                               %(        to install. Alternatively, provide a url: field.\n)
        end
      end
    end
